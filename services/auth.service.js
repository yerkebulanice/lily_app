...
exports.signup = async (req, res, next) => {
  ...
  const newUser = new User({
    ...
    role,
    acceptTerms,
    profileImage: req.file.filename ? `/static/uploads/users/${req.file.filename}` : user.profileImage,
  });
...
exports.login = async (req, res, next) => {
  ...
  try {
    ...
    // 401 Unauthorized
    if (!user) {
      return Response({}, false, true, 'UNKNOW: Auth Failed (Invalid Credentials)', 401);
    }
    ...
    if (!isPasswordCorrect) {
      return Response({}, false, true, 'PASSWORD: Auth Failed (Invalid Credentials)', 401);
    }
